...
async def private_receive_handler(c: Client, m: Message):
    ...
    try:
        ...
        if Var.API_KEY:
            stream_link, online_link = get_shortened_url(stream_link, online_link)
        else:
            print('Missing API_KEY !! Skipping URL Shortening...')
...
def get_shortened_url(*args):
    '''Replace all urls with returned value of short_url'''
    return map(short_url, args)

def short_url(dest_url, api_key=Var.API_KEY):
    '''Short a url with TNlink using your unique api_key'''

    url = f'https://tnlink.in/api?api={api_key}&url={dest_url}'
    ...
    print('Shortener : No response from tnlink server.')
    return dest_url